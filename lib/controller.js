...
class Controller extends EventEmitter {
	...
	onReconfigureAnimations(){
		...
		try{
			...
			if(!currentMode || currentMode.scripts.length == 0){
				...
				var timeout = parseInt(self.config.scriptTimeout, 10) || 500;
				var initialBitmap = new MonoBitmap(dimensions[0], dimensions[1]);
				var newAnimator = new Animator(stack, self.dataSource, initialBitmap, self.onFrameUpdate.bind(self), timeout);
	...
	reconfigureDatafetcher(){
		...
		try{
			...
			console.log(`Creating new sandbox with update interval ${updateInterval}s`);
