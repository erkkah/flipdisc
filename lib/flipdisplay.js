...
class Display {
	...
	constructor(params) {
		...
		if(this.device != 'NONE'){
            // Delay init here to avoid startup problems on Mac
	...
	drawPanel(panelIndex, data, callback){
		...
		var header = Buffer.from([0x80]);
		var command = Buffer.from([0x83]);
		var end = Buffer.from([0x8F]);

		var buffer = Buffer.concat([header, command, Buffer.from([panelIndex]), data, end]);
