...
class SandboxedClassModule extends EventEmitter {
	 ...
	 * @param [callback] {Function} - Called with (err, result) on completed of failed construction
	 */
	constructor(childmod, args, callback){
		super();
		...
		if(callback){
			this.callbacks.set(this.callID, callback);
		}
 ...
 */
	...
	onChildMessage(message){
		...
		if(message.result){
			if(callback){
				// Return "this" on successful construction
				var result = (message.method == 'constructor' ? this : message.result);
			...
			if(callback){
				// Return "this" on failed construction
				var result = (message.method == 'constructor' ? this : message.result);
